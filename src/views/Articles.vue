<template>
    <div class="articles-container">
      <div class="articles-header">
        <h2>Блог</h2>
        <button class="add-article" @click="openEditModal()">
      ...
      <div v-for="article in articles" :key="article.id" class="article-card">
        ...
        <div class="article-actions">
          <button class="edit-btn" @click="openEditModal(article.id)">
        ...
        <div class="likes-dislikes">
          <button class="like-btn" @click="likeArticle(article.id)">
            👍 {{ article.likes }}
          </button>
          <button class="dislike-btn" @click="dislikeArticle(article.id)">
            👎 {{ article.dislikes }}
          </button>
        </div>
  ...
  const saveArticle = (articleData: { title: string; content: string }) => {
    if (editingArticleId.value !== null) {
      articleStore.updateArticle(editingArticleId.value, articleData);
    } else {
  ...
  const handleAddComment = ({ articleId, comment }: { articleId: number; comment: any }) => {
  ...
  const dislikeArticle = (articleId: number) => {
    const article = articles.value.find((a) => a.id === articleId);
    if (article) {
      article.dislikes += 1;
    }
